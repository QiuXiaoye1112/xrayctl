...
xrayctl 可以与 sbctl/sing-box 同时安装。两者使用独立的命令、服务、配置、metadata、备份和 Certbot 环境。创建或修改 Xray 入站时，xrayctl 还会读取现有 sing-box 监听端口及 sbctl Hysteria2 UDP 端口跳跃范围，拒绝选择会冲突的端口。
...
- 用户管理页会直接显示 UUID 或密码，请避免在录屏、截图和共享终端中泄露。
